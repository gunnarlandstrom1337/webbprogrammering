...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById('searchbutton').addEventListener("click", function (e) {
        //alert("You clicked: "+document.getElementById("searchProgram").value);
    ...
    async function openProgram(channelid) {
        const headerTemp = await fetch(`https://api.sr.se/api/v2/channels/${channelid}?format=json`);
        const headerData = await headerTemp.json();
        //console.log(headerData);

        const musicTemp = await fetch(`http://api.sr.se/api/v2/playlists/rightnow?channelid=${headerData.channel.id}&format=json`);
    ...
    document.getElementById("info").innerHTML = `<h2>${channelHeader}</h2><h3>${channelTopic}</h3><hr>${musicInfoPrev}<br/>${musicInfoCurrent}<br/>${musicInfoNext}`;
...
async function openProgramTableau(channelid){
      const tableTemp = await fetch(`https://api.sr.se/api/v2/scheduledepisodes?channelid=${channelid}&format=json`);
        ...
        for (var i = 0; i < tableauData.schedule.length; i++ ){
            ...
            document.getElementById("info").innerHTML += `<h2>${tableauData.schedule[i].title}</h2><h3>${tableauData.schedule[i].description}</h3>${new Date(Number(tempDate)).toTimeString()}<hr/>`;
